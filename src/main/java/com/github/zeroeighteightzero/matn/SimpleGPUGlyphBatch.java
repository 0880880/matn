 ...
 * @see Batch
 */
public class SimpleGPUGlyphBatch implements Disposable, GPUGlyphBatch {
    ...
    /**
     * Constructs a new SimpleGPUGlyphBatch with a size of 1000, one buffer, and the default shader.
     */
    public SimpleGPUGlyphBatch() {
        this(1000);
    }

    /**
     * Constructs a new SimpleGPUGlyphBatch. Sets the projection matrix to an orthographic projection with y-axis point upwards, x-axis
     ...
     */
    public SimpleGPUGlyphBatch(int size) {
        // 32767 is max vertex index, so 32767 / 4 vertices per sprite = 8191 sprites max.
        if (size > 8191) throw new IllegalArgumentException("Can't have more than 8191 sprites per batch: " + size);

        currentDataType = (Gdx.gl30 != null) ? Mesh.VertexDataType.VertexBufferObjectWithVAO : Mesh.VertexDataType.VertexBufferObject;
    ...
    /**
     * Returns a new instance of the default shader used by SimpleGPUGlyphBatch for GL2 when no shader is specified.
     */
    ...
    public void begin() {
        if (drawing) throw new IllegalStateException("SimpleGPUGlyphBatch.end must be called before begin.");
        renderCalls = 0;
    ...
    public void end() {
        if (!drawing) {
            throw new IllegalStateException("SimpleGPUGlyphBatch.begin must be called before end.");
        }
        if (idx > 0) {
            flush();
        }
    ...
    public void drawGlyph(GPUGlyph glyph, float size, int minX, int minY, int maxX, int maxY, float x, float y) {
        if (!drawing) {
            throw new IllegalStateException("SimpleGPUGlyphBatch.begin must be called before draw.");
        }
